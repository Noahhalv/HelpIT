import httpStatusCodes from '../utils/httpStatusCodes.json' with { type: "json" };

if (httpStatusCodes) {console.log('Successfully loaded HTTP status codes')}
...
function send(res, statusCode, data, error) {
    const message = (`Status ${statusCode.toString()}: ${httpStatusCodes[statusCode.toString()]}`);
    console.log('\nResponding with:', statusCode, message, '\nhttpStatusCode:', httpStatusCodes[statusCode.toString()]);
